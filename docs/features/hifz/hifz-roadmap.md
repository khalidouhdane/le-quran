...
- [ ] Migrate hifz data from SharedPreferences to **SQLite**
- [ ] Implement `MemoryProfile` data model with all fields
...
- [ ] Multi-profile table: `profiles`, `progress_records`, `session_history`
- [ ] Profile CRUD operations (create, read, update, delete)
- [ ] Active profile switching

### 1.2 Assessment Wizard
- [ ] Build 9-screen onboarding flow
...
- [ ] Profile-to-parameter mapping (encoding × retention → framework params)
...
- [ ] Implement daily plan generation: Sabaq + Sabqi + Manzil assignments
...
- [ ] Manzil auto-rotation across completed juz (user can add/remove)
- [ ] Daily schedule JSON structure persisted in SQLite
- [ ] Plan override: user can edit today's assigned content
- [ ] Offline marking: "I already did this" (individual phase or all)

### 1.4 Dashboard (Home) Redesign
- [ ] Replace current Home screen with Hifz Dashboard
...
- [ ] **With profile:** Today's Plan card, Progress card, Werd card, Ayah of the Day
- [ ] **Without profile:** CTA card ("Start Your Hifz Journey"), Werd, Ayah
- [ ] Profile selector (quick switch)

### 1.5 Session Screen — Physical Quran Mode
- [ ] Pre-session screen: plan review, offline marking, estimated time
...
- [ ] Control panel: timer, rep counter, audio controls, step indicator
...
- [ ] Phase progression: Sabaq → Sabqi → Manzil (skippable, reorderable)
- [ ] Self-assessment after each phase (Strong / Okay / Needs Work)
- [ ] Session complete screen with summary + tomorrow's preview
- [ ] Session state persistence (pause and resume)

### 1.6 Progress Tracking
- [ ] Page-level progress storage (memorized / learning / reviewing / not started)
- [ ] Juz-level progress bars (% indicator)
- [ ] Progress detail screen with Pages (default) and Surahs tabs
...
- [ ] Active day streak counter (total, not consecutive)

### 1.7 Bottom Navigation Revamp
- [ ] Restructure to 5 tabs: Dashboard / Practice / Read / Listen / Profile
...
- [ ] Keep existing Read screen functionality intact
- [ ] Keep existing Audio screen as Listen tab
- [ ] Practice tab: placeholder with "Coming in next update" (or basic structure)
- [ ] Profile tab: settings, profile management, reassessment option

### 1.8 Missed Day Handling
- [ ] Detect missed days on app open
- [ ] Show compassionate re-entry screen: review-only suggestion
...
- [ ] User can accept suggestion, continue normal plan, or customize

### 1.9 Notifications
- [ ] Daily session reminder at user's preferred time
- [ ] Tap notification → opens directly into pre-session screen
  - 📄 Reference: [user-flows.md](file:///c:/Users/khali/OneDrive/Bureau/Quran%20App/docs/features/hifz/user-flows.md) § Flow 12
- [ ] Smart: skip notification if session already completed today
...
- [ ] SM-2 SRS engine (interval × rating modifier)
...
- [ ] Deck generation from memorized content (never from un-memorized)
...
- [ ] Practice tab UI: cards due count, start review, accuracy stats
...
- [ ] Import `Waqar144/Quran_Mutashabihat_Data` JSON dataset
...
- [ ] Enrich with category/difficulty metadata
- [ ] Implement 4 practice modes:
...
- [ ] Integration triggers: alert during memorization, inject into flashcard deck
...
- [ ] "X cards due" indicator on Dashboard → Today's Plan card
- [ ] Quick flashcard round at end of sessions (optional)
- [ ] Practice tab fully populated with flashcards + mutashabihat sections
...
- [ ] Show verse translations during sessions (Quran.com API, already integrated)
...
### 3.2 Brief Tafsir
- [ ] Integrate Tafsir al-Muyassar via Quran.com API
- [ ] "Meaning" button during sessions → shows brief tafsir for current verse
- [ ] "Detailed" button → shows scholar-level tafsir
...
### 3.3 Asbab al-Nuzul
- [ ] Import sample from `mostafaahmed97/asbab-al-nuzul-dataset` (JSON per surah)
- [ ] "Context" card during sessions → shows reason for revelation (when available)
...
- [ ] Thematic overview before starting a new surah in Sabaq phase
- [ ] Meccan/Medinan classification, key themes, key stories
...
- [ ] Restrict reading canvas to assigned content only during sessions
- [ ] Re-use existing `ReadingCanvas` widget with scoping layer
...
- [ ] Floating timer, rep counter, phase indicator on reading canvas
- [ ] Step navigation (skip/done) as bottom overlay

### 4.3 Mode Switching
- [ ] Toggle physical ↔ digital mid-session
- [ ] State persists across switches (reps, timer, phase position)

### 4.4 Verse-level Audio Sync
- [ ] Highlight active verse during audio playback in session
- [ ] Use existing audio timing data from Quran.com API
...
- [ ] Weekly pattern analysis: completion rate, self-assessment distribution
...
- [ ] Suggestion cards on dashboard (increase/decrease load, adjust review)
  - 📄 Reference: [user-flows.md](file:///c:/Users/khali/OneDrive/Bureau/Quran%20App/docs/features/hifz/user-flows.md) § Flow 10
- [ ] User accepts or dismisses — nothing changes automatically

### 5.2 Smart Notifications
- [ ] "You haven't reviewed Juz 30 in 5 days" reminders
- [ ] Struggle detection: consistently weak sections → add flashcards automatically

### 5.3 Performance Analytics
- [ ] Weekly/monthly hifz reports (pages memorized, retention rate, streaks)
- [ ] Progress pace calculation ("At this pace, you'll finish in X months")
...
- [ ] Invite friend to see your streaks and progress
- [ ] Mutual encouragement — no ranking, just visibility

### 6.2 Teacher Mode
- [ ] Share progress report with a teacher/mentor via link or PDF
- [ ] Teacher can optionally assign specific content

### 6.3 Community Milestones
- [ ] Celebrate juz/khatm completions with shareable cards
- [ ] Optional, opt-in leaderboards for competitive motivation
